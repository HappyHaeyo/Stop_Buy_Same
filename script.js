...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tabBtns = {
        ...
        'list': document.getElementById('tab-btn-list'),
        'analysis': document.getElementById('tab-btn-analysis'),
        'deco': document.getElementById('tab-btn-deco')
    };
    const views = {
        'home': document.getElementById('view-home'),
        'list': document.getElementById('view-list'),
        'analysis': document.getElementById('view-analysis'),
        'deco': document.getElementById('view-deco')
    };

    function switchTab(target) {
        // 모든 뷰 숨기고 버튼 비활성화
        Object.values(views).forEach(el => el.classList.add('hidden-tab'));
        Object.values(tabBtns).forEach(el => el.classList.remove('active'));

        // 선택된 뷰 보이고 버튼 활성화
        if (views[target]) views[target].classList.remove('hidden-tab');
        if (tabBtns[target]) tabBtns[target].classList.add('active');
        
        // 차트 리사이즈 이슈 방지를 위해 차트 업데이트 (Home 탭일 경우)
        if (target === 'home') {
            setTimeout(() => { updateAnalysis(); updateScatterChart(); }, 50);
        }
    }
                ...
                img.onload = function () {
                    try {
                        ...
                        if (selectBox) {
                            ...
                            // selectBox.classList.add('bg-rose-100'); // 기존 효과 제거 (배경색 충돌 방지)
...
function updateAnalysis() {
    ...
    // if (!section) return; // 탭 때문에 section이 hidden일 수 있음, 그래도 차트는 그려야 할 수 잇음 (하지만 DOM은 존재)
    
    const validData = lipsticks.filter(l => l.personalColor !== '잘 모름');
    // if (validData.length === 0) { section.classList.add('hidden'); return; } // 차트 영역을 아예 숨기는 로직은 주석 처리 (레이아웃 유지)
    ...
    if (validData.length > 0) {
        ...
        const analysisText = document.getElementById('analysisText');
        if (analysisText) analysisText.innerHTML = "데이터가 부족해요!";
    }
}
...
function render(filter='all') {
    ...
    filtered.forEach(lip => {
        const html = `
        <div class="item-card flex gap-3 items-center relative group fade-in">
            ...
            <button onclick="deleteItem(${lip.id})" class="text-stone-300 hover:text-red-500 p-2 opacity-0 group-hover:opacity-100 transition absolute right-2 top-2"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
        </div>`;
